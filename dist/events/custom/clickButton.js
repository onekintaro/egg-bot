...
// Dependencies
const { Embed } = require('../../utils'), { Collection, PermissionsBitField: { Flags: PermFlag } } = require('discord.js'), { ChannelType } = require('discord-api-types/v10'), cooldowns = new Collection(), Event = require('../../structures/Event');
/**
 * Click button event
    ...
    */
    run(bot, button) {
        return __awaiter(this, void 0, void 0, function* () {
            const { customId: ID, guildId, channelId, member } = button, guild = bot.guilds.cache.get(guildId), channel = bot.channels.cache.get(channelId);
            ...
            if (ID == 'crt_ticket') {
                ...
                if (guild.roles.cache.get(guild.settings.TicketSupportRole))
                    perms.push({ id: guild.settings.TicketSupportRole, allow: [PermFlag.SendMessages, PermFlag.ViewChannel] });
                console.log(perms);
                // create channel
                try {
                    const c = yield guild.channels.create({ name: `ticket-${member.user.id}`, type: ChannelType.GuildText,
                    ...
                    const embed = new Embed(bot, guild)
                        .setColor(0xFF5555)
                        .addFields({ name: guild.translate('ticket/ticket-create:FIELD1', { USERNAME: member.user.tag }), value: guild.translate('ticket/ticket-create:FIELDT') }, { name: guild.translate('ticket/ticket-create:FIELD2'), value: guild.translate('misc:NO_REASON') })
                        .setTimestamp();
                    ...
                    yield bot.emit('ticketCreate', channel, embed);
                }
                catch (err) {
                    console.log(err);
