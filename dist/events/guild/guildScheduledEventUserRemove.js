    ...
    */
    run(bot, guildEvent, user) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            // For debugging
            if (bot.config.debug)
                bot.logger.debug(`User: ${user.tag} has left event: ${guildEvent.name} in guild: ${guildEvent.guild.id}.`);
            // Get server settings / if no settings then return
            const settings = guildEvent.guild.settings;
            if (Object.keys(settings).length == 0)
                return;
            // Check if event guildEventCreate is for logging
            if (((_a = settings.ModLogEvents) === null || _a === void 0 ? void 0 : _a.includes('EVENTUSERREMOVE')) && settings.ModLog) {
                const embed = new Embed(bot, guildEvent.guild)
                    .setDescription([
                    `${user.tag} left event: [${guildEvent.name}](${guildEvent})`,
                    '',
                    `There are still ${(_b = guildEvent.userCount) !== null && _b !== void 0 ? _b : 0} users subscribed to the event.`,
                ].join('\n'))
                    .setColor(15158332)
