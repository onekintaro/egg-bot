...
class ChannelUpdate extends Event {
     ...
     * @readonly
    */
    run(bot, oldChannel, newChannel) {
        var _a, _b;
        return __awaiter(this, void 0, void 0, function* () {
            // For debugging
            if (bot.config.debug)
                bot.logger.debug(`Channel: ${newChannel.type == ChannelType.DM ? newChannel.recipient.tag : newChannel.name} has been updated${newChannel.type == ChannelType.DM ? '' : ` in guild: ${newChannel.guild.id}`}. (${ChannelType[newChannel.type]})`);
            // Get server settings / if no settings then return
            const settings = (_a = newChannel.guild) === null || _a === void 0 ? void 0 : _a.settings;
            if (Object.keys(settings).length == 0)
                return;
            // Check if event channelDelete is for logging
            if (((_b = settings.ModLogEvents) === null || _b === void 0 ? void 0 : _b.includes('CHANNELUPDATE')) && settings.ModLog) {
                ...
                if (oldChannel.name != newChannel.name) {
                    embed = new Embed(bot, newChannel.guild)
                        ...
                        .addFields({ name: 'Old:', value: `${oldChannel.name}`, inline: true }, { name: 'New:', value: `${newChannel.name}`, inline: true })
                        .setTimestamp();
                    updated = true;
                }
                ...
                if (oldChannel.topic != newChannel.topic) {
                    embed = new Embed(bot, newChannel.guild)
                        ...
                        .setFooter({ text: `ID: ${newChannel.id}` })
                        .setAuthor({ name: newChannel.guild.name, iconURL: newChannel.guild.iconURL() })
                        .addFields({ name: 'Old:', value: `${oldChannel.topic ? oldChannel.topic : '*empty topic*'}`, inline: true }, { name: 'New:', value: `${newChannel.topic ? newChannel.topic : '*empty topic*'}`, inline: true })
                        .setTimestamp();
                    updated = true;
                }
                if (oldChannel.rtcRegion != newChannel.rtcRegion) {
                    embed = new Embed(bot, newChannel.guild)
                        ...
                        .setFooter({ text: `ID: ${newChannel.id}` })
                        .setAuthor({ name: newChannel.guild.name, iconURL: newChannel.guild.iconURL() })
                        .addFields({ name: 'Old:', value: `${oldChannel.rtcRegion}`, inline: true }, { name: 'New:', value: `${newChannel.rtcRegion}`, inline: true })
                        .setTimestamp();
                    updated = true;
                }
                ...
                if (permDiff.size) {
                    ...
                    for (const permID of permDiff.keys()) {
                        ...
                        if (oldPerm.type == 'member' || newPerm.type == 'member') {
                            member = yield newChannel.guild.members.fetch(newPerm.id || oldPerm.id);
                        }
                ...
                if (updated) {
                    ...
                    try {
                        const modChannel = yield bot.channels.fetch(settings.ModLogChannel).catch(() => bot.logger.error(`Error fetching guild: ${newChannel.guild.id} logging channel`));
                        if (modChannel && modChannel.guild.id == newChannel.guild.id)
