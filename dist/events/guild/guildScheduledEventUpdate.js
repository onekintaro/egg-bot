...
class GuildScheduledEventUpdate extends Event {
     ...
     * @readonly
    */
    run(bot, oldGuildEvent, newGuildEvent) {
        var _a;
        return __awaiter(this, void 0, void 0, function* () {
            // For debugging
            if (bot.config.debug)
                bot.logger.debug(`Event: ${newGuildEvent.name} has been updated in guild: ${newGuildEvent.guild.id}.`);
            // Get server settings / if no settings then return
            const settings = newGuildEvent.guild.settings;
            if (Object.keys(settings).length == 0)
                return;
            // Check if event guildEventCreate is for logging
            if (((_a = settings.ModLogEvents) === null || _a === void 0 ? void 0 : _a.includes('EVENTUPDATE')) && settings.ModLog) {
                ...
                try {
                    const modChannel = yield bot.channels.fetch(settings.ModLogChannel).catch(() => bot.logger.error(`Error fetching guild: ${newGuildEvent.guild.id} logging channel`));
                    if (modChannel && modChannel.guild.id == newGuildEvent.guild.id)
