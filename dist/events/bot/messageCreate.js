...
// Dependencies
const { Collection, PermissionsBitField, EmbedBuilder } = require('discord.js'), { time: { getReadableTime }, functions: { genInviteLink } } = require('../../utils'), { TagsSchema } = require('../../database/models'), AutoModeration = require('../../helpers/autoModeration'), LevelManager = require('../../helpers/levelSystem'), Event = require('../../structures/Event');
/**
    ...
    run(bot, message) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (message.author.bot)
                return;
            // Get server settings
            const settings = (_b = (_a = message.guild) === null || _a === void 0 ? void 0 : _a.settings) !== null && _b !== void 0 ? _b : require('../../assets/json/defaultGuildSettings.json');
            ...
            if (new RegExp(`/<@(!?)${bot.user.id}>/g`).test(message.content)) {
                const embed = new EmbedBuilder()
                    ...
                    .addFields({ name: message.translate('events/message:LINKS'),
                    value: [
            ...
            if ([settings.prefix, `<@!${bot.user.id}>`].find(p => message.content.startsWith(p))) {
                ...
                if (!cmd && message.content.startsWith(`<@!${bot.user.id}>`)) {
                    ...
                    if (tag) {
                        const response = yield TagsSchema.find({ guildID: message.guild.id, name: tag });
                ...
                if (!settings.plugins.includes(cmd.help.category) && cmd.help.category != 'Host')
                    return;
                ...
                bot.commandsUsed++;
                if (bot.config.debug)
                    bot.logger.debug(`Command: ${cmd.help.name} was ran by ${message.author.tag}${!message.guild ? ' in DM\'s' : ` in guild: ${message.guild.id}`}.`);
                ...
                if (settings.plugins.includes('Moderation')) {
                    try {
                        const moderated = yield new AutoModeration(bot, message).check();
                        // This makes sure that if the auto-mod punished member, level plugin would not give XP
                        if (settings.plugins.includes('Level') && !moderated)
                            return new LevelManager(bot, message).check();
                    }
                    catch (err) {
                        console.log(err);
