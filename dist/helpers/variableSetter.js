...
module.exports = (string, member, channel, guild) => {
    const regex = /{(server|channel|user|member)(.[a-zA-Z]*)?}/g, chars = string.match(regex);
    let x = 0;
    if ((chars === null || chars === void 0 ? void 0 : chars.length) >= 1) {
        while (x < chars.length) {
            ...
            switch (type[0]) {
                case 'server': {
                    ...
                    if (constants[type].includes(value))
                        string = string.replace(chars[x], guild[value]);
                    break;
                }
                case 'channel': {
                    const type2 = chars[x].replace(`{${type[0]}`, '');
                    ...
                    // Replace variable with value
                    if (constants[type].includes(value))
                        string = string.replace(chars[x], channel[value]);
                    break;
                }
                case 'user': {
                    const type2 = chars[x].replace(`{${type[0]}`, '');
                ...
                case 'member': {
                    ...
                    // Replace variable with value
                    if (constants[type].includes(value))
                        string = string.replace(chars[x], member[value]);
                    break;
                }
            }
            x++;
