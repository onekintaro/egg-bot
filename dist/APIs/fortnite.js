...
const fetch = require('node-fetch'), URL = 'https://api.fortnitetracker.com/v1', modes = {
    p2: 'solo',
    p10: 'duo',
    p9: 'squad',
...
class FortniteAPI {
    ...
    user(username, platform = 'pc') {
        return __awaiter(this, void 0, void 0, function* () {
            if (!username)
                throw new Error('You must supply a username');
            if (typeof username !== 'string')
                throw new TypeError(`Username expects a string, ${typeof username} given`);
            if (typeof platform !== 'string')
                throw new TypeError(`Platform expects a string, ${typeof platform} given`);
            const data = yield fetch(`${URL}/profile/${platform}/${encodeURIComponent(username)}`, {
...
class Profile {
   ...
   */
    constructor(profile) {
        ...
        this.stats.lifetime = profile.lifeTimeStats.map(stat => new Stat(stat)).reduce((a, b) => (Object.assign(Object.assign({}, a), b)));
...
class Stat {
   ...
   */
    constructor(stat) {
        ...
        switch (stat.key) {
            case 'Score':
                this.score = stat.value;
                break;
            case 'K/d':
                this.kd = stat.value;
                break;
            case 'Matches Played':
                this.matches = stat.value;
                break;
            case 'Kills':
                this.kills = stat.value;
                break;
            case 'Wins':
                this.wins = stat.value;
                break;
            case 'Top 3s':
                this.top_3 = stat.value;
                break;
            case 'Top 5s':
                this.top_5 = stat.value;
                break;
            case 'Top 6s':
                this.top_6 = stat.value;
                break;
            case 'Top 12s':
                this.top_12 = stat.value;
                break;
            case 'Top 25s':
                this.top_25 = stat.value;
                break;
        }
    }
}
