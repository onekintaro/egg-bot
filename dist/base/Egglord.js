...
// Dependencies
const { ActivityType, Client, Collection, GatewayIntentBits: FLAGS, Partials, PermissionsBitField: { Flags: PermissionFlag } } = require('discord.js'), { GuildSchema } = require('../database/models'), GiveawaysManager = require('./Giveaway-Manager'), path = require('path'), { promisify } = require('util'), 
// AudioManager = require('./Audio-Manager'),
Reddit = require('../APIs/reddit.js'), readdir = promisify(require('fs').readdir);
/**
...
class Egglord extends Client {
    ...
    */
    DeleteGuild(guild) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                yield GuildSchema.findOneAndRemove({ guildID: guild.id });
                return true;
            }
            catch (err) {
    ...
    */
     ...
     * @returns {array}
    */
    loadInteractionGroup(category) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const commands = (yield readdir('./src/commands/' + category + '/')).filter((v, i, a) => a.indexOf(v) === i);
                const arr = [];
     ...
     * @returns {?array}
    */
    deleteInteractionGroup(category, guild) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const commands = (yield readdir('./src/commands/' + category + '/')).filter((v, i, a) => a.indexOf(v) === i);
                const arr = [];
    ...
    */
    translate(key, args, locale) {
        if (!locale)
            locale = require('../assets/json/defaultGuildSettings.json').Language;
