...
class GiveawaysManager extends EventEmitter {
    ...
    constructor(client, options, init = true) {
        super();
        if (!(client === null || client === void 0 ? void 0 : client.options))
    ...
    generateMainEmbed(giveaway, lastChanceEnabled = false) {
        ...
        const embed = new EmbedBuilder()
            ...
            : lastChanceEnabled
                ...
                : giveaway.embedColor)
            .setFooter({
            text: (_a = giveaway.messages.embedFooter.text) !== null && _a !== void 0 ? _a : (typeof giveaway.messages.embedFooter === 'string' ? giveaway.messages.embedFooter : ''),
            iconURL: giveaway.messages.embedFooter.iconURL,
        })
            .setDescription((giveaway.pauseOptions.isPaused
            ...
            '\n' +
            giveaway.messages.drawing.replace('{timestamp}', giveaway.endAt === Infinity
    ...
    generateEndEmbed(giveaway, winners) {
        ...
        const strings = {
            ...
            title: (_a = giveaway.fillInString(giveaway.messages.title)) !== null && _a !== void 0 ? _a : giveaway.fillInString(giveaway.prize),
        };
    ...
    */
    start(channel, options) {
        ...
        return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
            ...
            if (!(channel === null || channel === void 0 ? void 0 : channel.id) || !channel.isTextBased()) {
            ...
            const reaction = yield message.react(giveaway.reaction);
            giveaway.message = reaction.message;
            this.giveaways.push(giveaway);
            yield this.saveGiveaway(giveaway.messageId, giveaway.data);
            resolve(giveaway);
        }));
    }
    /**
    ...
    delete(messageId, doNotDeleteMessage = false) {
        ...
        return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
            ...
            if (!doNotDeleteMessage) {
                (_a = giveaway.message) !== null && _a !== void 0 ? _a : (giveaway.message = yield giveaway.fetchMessage().catch(() => null));
                (_b = giveaway.message) === null || _b === void 0 ? void 0 : _b.delete();
            }
            this.giveaways = this.giveaways.filter((g) => g.messageId !== messageId);
            yield this.deleteGiveaway(messageId);
    ...
    deleteGiveaway(messageId) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield GiveawaySchema.deleteOne({ messageId }).exec();
    ...
    getAllGiveaways() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            return yield GiveawaySchema.find().lean().exec();
        });
    }
    /**
    ...
    editGiveaway(messageId, giveawayData) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                yield GiveawaySchema.updateOne({ messageId }, giveawayData).exec();
                return true;
            }
            catch (e) {
                console.log(e);
    ...
    saveGiveaway(messageId, giveawayData) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                yield GiveawaySchema.create(giveawayData);
                return true;
            }
            catch (e) {
                console.log(e);
    ...
    _checkGiveaway() {
        if (this.giveaways.length <= 0)
            return;
        this.giveaways.forEach((giveaway) => __awaiter(this, void 0, void 0, function* () {
            var _a;
            ...
            if (giveaway.ended) {
                if (Number.isFinite(this.options.endedGiveawaysLifetime) &&
                    ...
                    yield this.deleteGiveaway(giveaway.messageId);
                }
                return;
            }
            // Third case: the giveaway is paused and we should check whether it should be unpaused
            if (giveaway.pauseOptions.isPaused) {
                if (!Number.isFinite(giveaway.pauseOptions.unpauseAfter) &&
                    ...
                    yield this.editGiveaway(giveaway.messageId, giveaway.data);
                }
                if (Number.isFinite(giveaway.pauseOptions.unpauseAfter) &&
            ...
            if (giveaway.remainingTime <= 0)
                return this.end(giveaway.messageId).catch(() => null);
            ...
            if (giveaway.lastChance.enabled && giveaway.remainingTime - giveaway.lastChance.threshold < DEFAULT_CHECK_INTERVAL) {
                setTimeout(() => __awaiter(this, void 0, void 0, function* () {
                    var _b, _c;
                    (_b = giveaway.message) !== null && _b !== void 0 ? _b : (giveaway.message = yield giveaway.fetchMessage().catch(() => null));
                    const embed = this.generateMainEmbed(giveaway, true);
                    yield ((_c = giveaway.message) === null || _c === void 0 ? void 0 : _c.edit({
                        content: giveaway.fillInString(giveaway.messages.giveaway),
                        embeds: [embed],
                        allowedMentions: giveaway.allowedMentions,
                    }).catch(() => null));
            ...
            if (!giveaway.message.embeds[0])
                yield giveaway.message.suppressEmbeds(false).catch(() => null);
    ...
    _handleRawPacket(packet) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const guild = this.client.guilds.cache.get(packet.d.guild_id) ||
                (yield this.client.guilds.fetch(packet.d.guild_id).catch(() => null));
            if (!guild || !guild.available)
                return;
            const member = yield guild.members.fetch(packet.d.user_id).catch(() => null);
            if (!member)
                return;
            const channel = yield this.client.channels.fetch(packet.d.channel_id).catch(() => null);
            if (!channel)
                return;
            const message = yield channel.messages.fetch(packet.d.message_id).catch(() => null);
            if (!message)
                return;
            const emoji = resolvePartialEmoji(giveaway.reaction);
            const reaction = message.reactions.cache.find((r) => { var _a; return [r.emoji.name, r.emoji.id].filter(Boolean).includes((_a = emoji === null || emoji === void 0 ? void 0 : emoji.id) !== null && _a !== void 0 ? _a : emoji === null || emoji === void 0 ? void 0 : emoji.name); });
            if (!reaction || reaction.emoji.name !== packet.d.emoji.name)
                return;
            if (reaction.emoji.id && reaction.emoji.id !== packet.d.emoji.id)
                return;
            if (packet.t === 'MESSAGE_REACTION_ADD') {
                if (giveaway.ended)
                    return this.emit('endedGiveawayReactionAdded', giveaway, member, reaction);
    ...
    _init() {
        return __awaiter(this, void 0, void 0, function* () {
            let rawGiveaways = yield this.getAllGiveaways();
            yield (this.client.readyAt ? Promise.resolve() : new Promise((resolve) => this.client.once('ready', resolve)));
            // Filter giveaways for each shard
            if (this.client.shard && this.client.guilds.cache.size) {
                const shardId = ShardClientUtil.shardIdForGuildId(this.client.guilds.cache.first().id, this.client.shard.count);
                rawGiveaways = rawGiveaways.filter((g) => shardId === ShardClientUtil.shardIdForGuildId(g.guildId, this.client.shard.count));
            }
