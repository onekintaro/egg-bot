    ...
    */
    callback(bot, interaction, guild, args) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const embed = this.createEmbed(bot, guild.settings, channel, (_a = args.get('command')) === null || _a === void 0 ? void 0 : _a.value, interaction.member.user);
    ...
    */
    createEmbed(bot, settings, channel, command, user) {
        if (!command) {
            ...
            if (!channel.guild)
                categories = categories.filter(c => !bot.commands.filter(cmd => cmd.help.category === c).first().conf.guildOnly);
            if (bot.config.ownerID.includes(user.id))
