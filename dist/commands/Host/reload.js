...
// Dependencies
const { promisify } = require('util'), readdir = promisify(require('fs').readdir), path = require('path'), { ApplicationCommandOptionType, PermissionsBitField: { Flags } } = require('discord.js'), Command = require('../../structures/Command.js');
/**
 * Reload command
 * @extends {Command}
*/
    ...
    */
    run(bot, message, settings) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!message.args[0])
                return message.channel.error('misc:INCORRECT_FORMAT', { EXAMPLE: settings.prefix.concat(message.translate('host/reload:USAGE')) });
            ...
            if (bot.commands.has(commandName) || bot.commands.get(bot.aliases.get(commandName))) {
                ...
                // reloads command
                try {
                    yield bot.unloadCommand(cmd.conf.location, cmd.help.name);
                    yield bot.loadCommand(cmd.conf.location, cmd.help.name);
