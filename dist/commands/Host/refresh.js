...
// Dependencies
const { ApplicationCommandType } = require('discord-api-types/v10'), { PermissionsBitField: { Flags } } = require('discord.js'), Command = require('../../structures/Command.js');
/**
 * Docs command
 * @extends {Command}
*/
    ...
    */
    run(bot, message, settings) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            if (!message.args[0])
                return message.channel.error('misc:INCORRECT_FORMAT', { EXAMPLE: settings.prefix.concat(message.translate('host/refresh:USAGE')) }).then(m => m.timedDelete({ timeout: 5000 }));
                ...
                case 'interactions': {
                    ...
                    for (const guild of [...bot.guilds.cache.values()]) {
                        ...
                        if (guild.id == bot.config.SupportServer.GuildID) {
                            const cmds = yield bot.loadInteractionGroup('Host', guild);
                            for (const cmd of cmds) {
                                cmd.defaultMemberPermissions = [Flags.Administrator];
                            }
                            if (Array.isArray(cmds))
                                cmdsToUpload.push(...cmds);
                        }
                        // get context menus
                        try {
                            yield ((_a = bot.guilds.cache.get(guild.id)) === null || _a === void 0 ? void 0 : _a.commands.set(cmdsToUpload));
