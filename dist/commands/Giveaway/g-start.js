...
// Dependencies
const { time: { getTotalTime } } = require('../../utils'), { ApplicationCommandOptionType, PermissionsBitField: { Flags } } = require('discord.js'), { ChannelType } = require('discord-api-types/v10'), Command = require('../../structures/Command.js');
/**
  ...
  */
    run(bot, message, settings) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (message.args.length <= 2)
                return message.channel.error('misc:INCORRECT_FORMAT', { EXAMPLE: settings.prefix.concat(message.translate('giveaway/g-start:USAGE')) });
            // Get time
            ...
            if (message.args.slice(2).join(' ').length >= 256)
                return message.channel.error('giveaway/g-start:PRIZE_TOO_LONG');
            // Start the giveaway
            try {
                yield bot.giveawaysManager.start(message.channel, {
                    duration: time,
    ...
    */
    callback(bot, interaction, guild, args) {
        var _a, _b;
        return __awaiter(this, void 0, void 0, function* () {
            const member = guild.members.cache.get(interaction.user.id), channel = guild.channels.cache.get((_b = (_a = args.get('channel')) === null || _a === void 0 ? void 0 : _a.value) !== null && _b !== void 0 ? _b : interaction.channelId), winners = args.get('winners').value, prize = args.get('prize').value;
            // Get time
            ...
            // Start the giveaway
            try {
                yield bot.giveawaysManager.start(channel, {
                    duration: time,
                    prize: prize,
                    winnerCount: winners,
                    hostedBy: member,
