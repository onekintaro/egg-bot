...
// Dependencies
const { WarningSchema } = require('../../database/models'), { ApplicationCommandOptionType, PermissionsBitField: { Flags } } = require('discord.js'), Command = require('../../structures/Command.js');
/**
...
class ClearWarning extends Command {
    ...
    */
    run(bot, message, settings) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!message.args[0])
                return message.channel.error('misc:INCORRECT_FORMAT', { EXAMPLE: settings.prefix.concat(message.translate('moderation/clear-warning:USAGE')) });
            ...
            try {
                ...
                if (message.args[1] - 1 <= warns.length) {
                    ...
                    yield WarningSchema.findByIdAndRemove(warns[message.args[1] - 1]._id);
                }
                else {
                    yield WarningSchema.deleteMany({ userID: members[0].user.id, guildID: message.guild.id });
                }
    ...
    */
    callback(bot, interaction, guild, args) {
        return __awaiter(this, void 0, void 0, function* () {
            const member = guild.members.cache.get(args.get('user').value), channel = guild.channels.cache.get(interaction.channelId), num = args.get('warn-num').value;
            // get warnings of user
            try {
                // find data
                const warns = yield WarningSchema.find({
                ...
                if (num - 1 <= warns.length) {
                    // Delete item from database as bot didn't crash
                    yield WarningSchema.findByIdAndRemove(warns[num - 1]._id);
                }
                else {
                    yield WarningSchema.deleteMany({ userID: member.user.id, guildID: guild.id });
                }
