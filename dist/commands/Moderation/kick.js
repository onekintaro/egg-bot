    ...
    */
    run(bot, message, settings) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!message.args[0])
                return message.channel.error('misc:INCORRECT_FORMAT', { EXAMPLE: settings.prefix.concat(message.translate('moderation/kick:USAGE')) });
            ...
            try {
                ...
                try {
                    const embed = new Embed(bot, message.guild)
                        ...
                        .addFields({ name: message.translate('moderation/kick:KICKED'), value: message.author.tag, inline: true }, { name: message.translate('misc:REASON'), value: reason, inline: true });
                    yield members[0].send({ embeds: [embed] });
                    // eslint-disable-next-line no-empty
                }
                catch (e) { }
                // kick user from guild
                yield members[0].kick({ reason: reason });
            ...
            try {
                ...
                try {
                    const embed = new Embed(bot, guild)
                        ...
                        .addFields({ name: guild.translate('moderation/kick:KICKED'), value: interaction.user.tag, inline: true }, { name: guild.translate('misc:REASON'), value: reason, inline: true });
                    yield member.send({ embeds: [embed] });
                    // eslint-disable-next-line no-empty
                }
                catch (e) { }
                // kick user from guild
                yield member.kick({ reason: reason });
