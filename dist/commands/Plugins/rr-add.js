...
// Dependencies
const Command = require('../../structures/Command.js'), { ReactionRoleSchema } = require('../../database/models'), { PermissionsBitField: { Flags } } = require('discord.js'), { Embed } = require('../../utils');
/**
  ...
  */
    run(bot, message, settings) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield ReactionRoleSchema.find({
                guildID: message.guild.id,
            }, (err, reacts) => __awaiter(this, void 0, void 0, function* () {
                var _a, _b, _c;
                // if an error occured
                if (err) {
                ...
                if (reacts.length >= 3 && !message.guild.premium) {
                    ...
                    if (patt.test(message.args[0])) {
                        ...
                        msgLink = yield ((_b = (_a = bot.guilds.cache.get(stuff[4])) === null || _a === void 0 ? void 0 : _a.channels.cache.get(stuff[5])) === null || _b === void 0 ? void 0 : _b.messages.fetch(stuff[6]));
                        if (!msgLink)
                            return message.channel.error('Incorrect message link.');
                        channel = msgLink.channel;
                    }
                    else {
                        channel = (_c = message.guild.channels.cache.get(message.args[0])) !== null && _c !== void 0 ? _c : message.channel;
                    }
                    ...
                    let roleMsgs;
                    try {
                        roleMsgs = yield message.channel.awaitMessages({
                            filter,
                            time: 60000,
                            max: 1,
                            errors: ['time'],
                    ...
                    if (!roles[0])
                        return message.channel.send('No roles entered');
                    ...
                    let emojiMsgs;
                    try {
                        emojiMsgs = yield message.channel.awaitMessages({
                            filter,
                            time: 60000,
                            max: 1,
                            errors: ['time'],
                    ...
                    // Validate the list of emoji for reaction roles
                    const emojiMsg = emojiMsgs.first(), emojis = this.parseEmojis(emojiMsg);
                    emojis.splice(emojis.length, roles.length);
                    // Make sure the correct number of emojis were entered
                    if (!emojis[0] || emojis.length < roles.length)
                        return message.channel.send(message.translate('plugins/rr-add:INCORRECT_EMOJIS'));
                    ...
                    if (patt.test(message.args[0])) {
                        ...
                        msg = yield channel.send({ embeds: [embed2] });
                    }
