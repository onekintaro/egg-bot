...
// Dependencies
const { MessageActionRow, MessageSelectMenu, PermissionsBitField: { Flags } } = require('discord.js'), Command = require('../../structures/Command.js');
const flags = {
    'en-US': '🏴󠁧󠁢󠁥󠁮󠁧󠁿',
    'fr-FR': '🇫🇷',
    'ar-EG': '🇪🇬',
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (message.args[0]) {
                ...
                if (!message.args[0] || !language) {
                    ...
                    yield updateLanguage(message.guild, language);
                }
            }
            else {
                ...
                const row = new MessageActionRow()
                    ...
                    .addOptions(options));
                // send message
                message.channel.send({ content: 'Please select a language for this server', components: [row] }).then((msg) => __awaiter(this, void 0, void 0, function* () {
                    const filter = (interaction) => interaction.customId === 'setlang' && interaction.user.id === message.author.id;
                    try {
                        const resp = yield msg.awaitMessageComponent({ filter, componentType: 'SELECT_MENU', time: 15000 });
                        yield updateLanguage(message.guild, bot.languages.find(l => l.name === resp.values[0]));
                        msg.delete();
                    }
                    catch (_a) {
                        msg.delete();
            ...
            function updateLanguage(guild, language) {
                return __awaiter(this, void 0, void 0, function* () {
                    try {
                        yield guild.updateGuild({ Language: language.name });
