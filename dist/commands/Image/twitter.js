    ...
    */
    run(bot, message, settings) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (message.args.join(' ').replace(/<@.?\d*?>/g, '').length == message.args.length)
                message.args.shift();
            // Get text
            const text = message.args.join(' ').replace(/<@.?\d*?>/g, '');
            // make sure text was entered
            if (message.args.length == 0)
                return message.channel.error('misc:INCORRECT_FORMAT', { EXAMPLE: settings.prefix.concat(message.translate('image/twitter:USAGE')) });
            // make sure the text isn't longer than 60 characters
            if (text.length >= 61)
                return message.channel.error('image/twitter:TOO_LONG');
            ...
            try {
                const json = yield fetch(encodeURI(`https://nekobot.xyz/api/imagegen?type=tweet&username=${members[0].user.username}&text=${text}`)).then(res => res.json());
                // send image in embed
    ...
    */
    callback(bot, interaction, guild, args) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                const json = yield fetch(encodeURI(`https://nekobot.xyz/api/imagegen?type=tweet&username=${member.user.username}&text=${text}`)).then(res => res.json());
