    ...
    run(bot, message, settings) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!Array.isArray(files))
                return;
            if (!files[1])
                return message.channel.error('misc:INCORRECT_FORMAT', { EXAMPLE: settings.prefix.concat(message.translate('image/whowouldwin:USAGE')) });
            ...
            try {
                const json = yield fetch(encodeURI(`https://nekobot.xyz/api/imagegen?type=whowouldwin&user1=${files[0]}&user2=${files[1]}`)).then(res => res.json());
                // send image in embed
    ...
    */
    callback(bot, interaction, guild, args) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                const json = yield fetch(encodeURI(`https://nekobot.xyz/api/imagegen?type=whowouldwin&user1=${member.user.displayAvatarURL({ format: 'png', size: 512 })}&user2=${member2.user.displayAvatarURL({ format: 'png', size: 512 })}`)).then(res => res.json());
