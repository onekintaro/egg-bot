    ...
    */
    run(bot, message, settings) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!text)
                return message.channel.error('misc:INCORRECT_FORMAT', { EXAMPLE: settings.prefix.concat(message.translate('image/clyde:USAGE')) });
            // make sure the text isn't longer than 70 characters
            if (text.length >= 71)
                return message.channel.error('image/clyde:TOO_LONG');
            ...
            try {
                const json = yield fetch(encodeURI(`https://nekobot.xyz/api/imagegen?type=clyde&text=${text}`)).then(res => res.json());
                // send image
